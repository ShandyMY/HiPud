...
export const loginAdmin = async (req, res) => {
    try {
        ...
        if (isEnvCredential || isLocalDevCredential) {
            ...
            const token = jwt.sign({ role: 'superadmin', username: username }, process.env.JWT_SECRET || 'rahasia_negara_123', { expiresIn: '24h' });
...
export const getDashboardStats = async (req, res) => {
    try {
        const [totalProducts, totalOrders, pendingOrders, pendingPaymentOrders, pendingVerificationOrders, totalReviews, pendingReviewsCount, uniqueCustomers, successfulOrders, recentOrders, activeProducts, pendingReviews] = await Promise.all([
